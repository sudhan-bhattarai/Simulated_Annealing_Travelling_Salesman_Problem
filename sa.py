        ...
        for i in range(self.num_iter):
            ...
            if new_tour not in self.explored_tour and temp >= self.temp_threshold:
                ...
                if d1 >= d2:                           ## if new solution is better than previous one, accept it right away
                    ...
                    delta = d2 - d1
                    probability = np.exp(-delta / temp)    ## calculate probability if new solution is not better than previous
                    if probability >= np.random.rand():   ## keep solution if probability is better than a random probability between 0 and 1.
                        current_tour = new_tour
                if dist.tour_cost(current_tour) <= dist.tour_cost(self.best_tour):
                    self.best_tour = current_tour
                    self.best_tempr = temp
                    self.iter_to_opt = i
                temp *= self.temp_fraction  ## reduce temperature for next iteration by pre-defined fraction.
...
print('\nSolution improved by SA is:',dist.tour_cost(nns.solution)- dist.tour_cost(tsp.best_tour))
...
def plotSA():
    ...
    plt.text(dist_list.index(min(dist_list))+1,min(dist_list)+0.5, 'SA Optimal:'+ ' '+ str(round(min(dist_list),2)))
    plt.axhline(y=dist.tour_cost(nns.solution), color='r', linestyle='-', label='NNS Optimal')
